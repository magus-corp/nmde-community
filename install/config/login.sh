...
if [ -d "/boot/loader/entries" ]; then # systemd-boot
  ...
  if ! grep -q splash /etc/cmdline.d/*.conf; then
    # Need splash, create the nmde file
    echo "splash" | sudo tee -a /etc/cmdline.d/nmde.conf
  fi
  if ! grep -q quiet /etc/cmdline.d/*.conf; then
    # Need quiet, create or append the nmde file
    echo "quiet" | sudo tee -a /etc/cmdline.d/nmde.conf
  fi
...
if [ "$(plymouth-set-default-theme)" != "nmde" ]; then
  sudo cp -r "$HOME/.local/share/nmde/default/plymouth" /usr/share/plymouth/themes/nmde/
  sudo plymouth-set-default-theme -R nmde
fi
...
if [ ! -f /etc/systemd/system/nmde-seamless-login.service ]; then
  cat <<EOF | sudo tee /etc/systemd/system/nmde-seamless-login.service
[Unit]
...
# Enable nmde-seamless-login.service only if not already enabled
if ! systemctl is-enabled nmde-seamless-login.service | grep -q enabled; then
  sudo systemctl enable nmde-seamless-login.service
fi
